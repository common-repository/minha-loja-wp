jQuery (function() {
    ...
    jQuery( '.bwec_close_cart' ).click( function() {
        var frete_calculado = jQuery( '#frete-calculado' ).val();
        if ( frete_calculado > 0 )
            return true;
        else {
            alert( 'Insira o seu CEP para calcular o frete antes de poder prosseguir.' );
            return false;
        }
...
jQuery(function() {
	...
	jQuery('textarea').resetDefaultValue(); // work with textarea too
        ...
        jQuery( '#search-button' ).click( function() {
            var s = jQuery.trim( jQuery( '#s' ).val() );

            if ( s == '' || s == 'Faça sua pesquisa' ) {
